...
#define GPIO_PORTB_PIN1      0x02    // Laser sensor input (PB1)
...
void PB1_LaserSensor_Init(void);
...
void PB1_LaserSensor_Init(void) {
    ...
    // ?? Change edge trigger based on your laser sensor’s output:
    // If the laser sensor gives LOW-to-HIGH pulse ? keep this line:
    GPIOB->IEV |= GPIO_PORTB_PIN1;    // Rising edge trigger
    // If it gives HIGH-to-LOW pulse ? use:
    // GPIOB->IEV &= ~GPIO_PORTB_PIN1; // Falling edge trigger

    GPIOB->IBE &= ~GPIO_PORTB_PIN1;
    GPIOB->ICR |= GPIO_PORTB_PIN1;
    GPIOB->IM  |= GPIO_PORTB_PIN1;
...
void TIMER3A_Handler(void) {
    char buffer[40];
    rpm = (pulse_count * 60) / 2;   // Adjust divisor if 1 pulse per rev
    pulse_count = 0;

    sprintf(buffer, "Laser RPM: %lu\n", rpm);
    UART0_TxString(buffer);

    TIMER3->ICR = 0x01;
}

// Laser sensor pulse detect
void GPIOB_Handler(void) {
